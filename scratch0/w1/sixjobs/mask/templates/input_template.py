...
mad.globals.ahcrab_l1b1 -= knob_settings['par_crab_L1B1']/100.*mad.globals.ahcrab_l1b1
mad.globals.ahcrab_l1b2 -= knob_settings['par_crab_L1B2']/100.*mad.globals.ahcrab_l1b2
mad.globals.ahcrab_r1b1 -= knob_settings['par_crab_R1B1']/100.*mad.globals.ahcrab_r1b1
mad.globals.ahcrab_r1b2 -= knob_settings['par_crab_R1B2']/100.*mad.globals.ahcrab_r1b2
mad.globals.avcrab_l5b1 -= knob_settings['par_crab_L5B1']/100.*mad.globals.avcrab_l5b1
mad.globals.avcrab_l5b2 -= knob_settings['par_crab_L5B2']/100.*mad.globals.avcrab_l5b2
mad.globals.avcrab_r5b1 -= knob_settings['par_crab_R5B1']/100.*mad.globals.avcrab_r5b1
mad.globals.avcrab_r5b2 -= knob_settings['par_crab_R5B2']/100.*mad.globals.avcrab_r5b2
...
if enable_crabs:
    ...
    # Slighlty change CCs voltage to make them appear as independent elements in fort.2
    elems = mad_track.elements
    
    ccs_b1 = []
    ccs_b2 = []
    planes    = ['h', 'v']
    positions = ['r1', 'l1', 'r5', 'l5']
    cc_types  = ['a', 'b']
    for plane in planes:
        for position in positions:
            for cc_type in cc_types:
                ccs_b1.append(f'acfga{plane}.4{cc_type}{position}.b1')
                ccs_b2.append(f'acfga{plane}.4{cc_type}{position}.b2')
    
    #for counter, cc in enumerate(ccs_b1):
    #    elems[cc]['volt'] += counter*1e-8
    import pandas as pd
    df_crabs_b1 = pd.DataFrame({'name':[cc for cc in ccs_b1],
    'freq': [elems[cc]['freq'] for cc in ccs_b1],
    'volt': [elems[cc]['volt'] for cc in ccs_b1],
    'position': [elems[cc]['at'] for cc in ccs_b1]
    })
    df_crabs_b1.to_pickle('df_crabs_b1.pickle')

    # All CCs are set to approx 0 in fc.2 when ramping. The real voltage is asigned by the DYNK block in sixtrack
    for counter, cc in enumerate(ccs_b1):
        elems[cc]['volt'] = counter*1e-8
...
mad_track._use('lhcb2')
mad_track.globals.on_bb_charge = 0
mad_track.twiss()
tdf = mad_track.get_twiss_df('twiss')
sdf = mad_track.get_summ_df('summ')
sdf.to_parquet('final_summ_BBOFF_B2.parquet')
tdf.to_parquet('final_twiss_BBOFF_B2.parquet')
